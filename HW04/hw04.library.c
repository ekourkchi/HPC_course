...
// This might not be an issue in the simulation, but working 
// with a real cluster, it becomes important
#if MODE == 0
  ...
  if (rank == 0) {
    ...
    // It does not go to the next step, until the next processor receives the message to start printing
  ...
  // defining new MPI communicators on each row and column
  ...
  // This is the outer-most loop of matrix multiplication
  k = 0;
  for (k=0; k<p2; k++) {
    ...
    // Doing actual matrix multipilication
    MatrixMultiplyAdd(C, buffer_A, buffer_B, n);

#else
// Don't do the multiplication, using the estimated flops
...
#if MODE < 2
// in the case of doing the actual multiplication, you can print C-matrix here 
  ...
  if(rank == 0) {
    ...
    // For very huge N, the follwoing function does not work perfectly
