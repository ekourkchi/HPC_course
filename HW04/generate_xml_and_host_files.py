...
if (len(sys.argv) != 2):
	...
	print >> sys.stderr, "  Will generate all topologies with the specified number of hosts\n"
	exit(1)
...
filename = "./cluster_crossbar_"+str(num_hosts)+".xml"
fh = open(filename, 'w')
fh.write(issueHead())
...
fh.write(issueTail())
fh.close()
print >> sys.stderr, "Cluster crossbar XML platform description file created: "+filename

###############################################################
# Generate Cluster Shared Backbone XML file
filename = "./cluster_backbone_"+str(num_hosts)+".xml"
fh = open(filename, 'w')
fh.write(issueHead())

fh.write("<cluster id=\"cluster\" prefix=\"host-\" suffix=\".hawaii.edu\" radical=\"0-"+str(num_hosts-1)+"\" power=\"1\" bw=\""+str(link_bandwidth)+link_bandwidth_unit+"\" lat=\""+str(link_latency)+"\" bb_bw=\"2.25GBps\" bb_lat=\"100us\"/>\n")
fh.write(issueTail())
fh.close()
print >> sys.stderr, "Cluster backbone XML platform description file created: "+filename
